...
import { User, Mail, Briefcase, TriangleAlert as AlertTriangle, FileText, UserCog, ChevronLeft, ChevronRight, Sparkles, Info, Zap, RefreshCw, PartyPopper } from "lucide-react";
...
const INITIAL: FormData = {
  traineeName: "", oracleId: "", personalEmail: "", workEmail: "",
  accountId: "", lobId: "", siteId: "", wave: "", hireDate: "",
  attritionCategory: "", subReason: "", severityLevel: "Low",
  totalMissedHours: 0, incidentDate: new Date().toISOString().split("T")[0],
  documentationRequired: false, escalationRequired: false,
  trainingManager: "", trainingManagerEmail: "", notes: "",
};
...
export default function SubmitCase() {
  ...
  const loadLookups = useCallback(async () => {
    setLoading(true);
    try {
      const [a, l, s] = await Promise.all([fetchAccounts(), fetchLOBs(), fetchSites()]);
      setAccounts(a); setLOBs(l); setSites(s);
    } catch (err) {
  ...
  function validateStep(): string | null {
    if (step === 1) {
      ...
      if (!form.personalEmail.trim() || !/\S+@\S+\.\S+/.test(form.personalEmail)) return "Valid personal email is required";
    ...
    if (step === 3) {
      ...
      if (!form.trainingManagerEmail.trim() || !/\S+@\S+\.\S+/.test(form.trainingManagerEmail)) return "Valid training manager email is required";
    }
    return null;
  }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const payload: CreateCasePayload = {
        traineeName: form.traineeName, oracleId: form.oracleId,
        personalEmail: form.personalEmail, workEmail: form.workEmail,
        account: accountTitle, lob: lobTitle, site: siteTitle,
        wave: form.wave, hireDate: form.hireDate,
        trainerName: user?.displayName || "", trainerEmail: user?.email || "",
        trainingManager: form.trainingManager, trainingManagerEmail: form.trainingManagerEmail,
        attritionCategory: form.attritionCategory, subReason: form.subReason,
        severityLevel: form.severityLevel, totalMissedHours: form.totalMissedHours,
        incidentDate: form.incidentDate, riskStatus, lifecycleStage,
        notes: form.notes, documentationRequired: form.documentationRequired,
        escalationRequired: form.escalationRequired, openedBy: user?.email || "",
      };
  ...
  if (success) {
    return (
      <div className="max-w-lg mx-auto mt-16 text-center animate-scale-in relative">
        {confetti && (
          <>
            {Array.from({ length: 50 }).map((_, i) => (
              <div
                ...
                style={{
                  ...
                  background: ["#00C4B4", "#0D2B45", "#2563EB", "#F59E0B", "#EF4444", "#22C55E"][i % 6],
        ...
        <div className="flex items-center justify-center gap-2 mb-2">
          ...
          <h2 className="font-barlow-condensed text-2xl font-bold text-gray-900 tracking-wide">CASE SUBMITTED</h2>
        </div>
        ...
        <div className="flex gap-3 justify-center">
          <button
            ...
            className="bg-gradient-teal hover:opacity-90 text-white px-5 py-2.5 rounded-xl text-sm font-medium transition-all shadow-glow-teal flex items-center gap-2 btn-press"
          >
            <RefreshCw className="w-4 h-4" /> Submit Another
          </button>
          <button
            onClick={() => navigate("/")}
            className="glass-card bg-white/90 backdrop-blur-xl border border-white/30 text-gray-700 hover:bg-white px-5 py-2.5 rounded-xl text-sm font-medium transition-all flex items-center gap-2 btn-press"
          >
  ...
  return (
    <div className={`max-w-5xl mx-auto animate-fade-up ${shake ? "animate-shake" : ""}`}>
      {/* Header */}
      <div className="mb-8">
        ...
        <h1 className="font-barlow-condensed text-3xl font-bold text-gray-900 tracking-wide">SUBMIT CASE</h1>
      ...
      <div className="glass-card bg-white/90 backdrop-blur-xl border border-white/30 rounded-2xl shadow-glass p-6 space-y-6">
        {/* Step 1: Trainee Info - Two Column */}
        {step === 1 && (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Tooltip content="Full name as per HR records" position="top">
                <FormField label="Trainee Full Name" required>
                  <input type="text" className={`${inputClass} teal-focus-glow`} value={form.traineeName} onChange={(e) => update("traineeName", e.target.value)} placeholder="First Last" />
                </FormField>
              </Tooltip>
              <Tooltip content="Unique Oracle employee identifier" position="top">
                <FormField label="Oracle ID" required>
                  <input type="text" className={`${inputClass} font-mono teal-focus-glow`} value={form.oracleId} onChange={(e) => update("oracleId", e.target.value)} placeholder="ORG-123456" />
                </FormField>
              </Tooltip>
              <Tooltip content="Personal email for emergency contact" position="top">
                <FormField label="Personal Email" required>
                  <input type="email" className={`${inputClass} teal-focus-glow`} value={form.personalEmail} onChange={(e) => update("personalEmail", e.target.value)} placeholder="trainee@personal.com" />
                </FormField>
              </Tooltip>
              <Tooltip content="Concentrix work email (optional)" position="top">
                <FormField label="Work Email">
                  <input type="email" className={`${inputClass} teal-focus-glow`} value={form.workEmail} onChange={(e) => update("workEmail", e.target.value)} placeholder="trainee@concentrix.com" />
                </FormField>
              </Tooltip>
              <Tooltip content="Training batch or cohort identifier" position="top">
                <FormField label="Wave">
                  <input type="text" className={`${inputClass} teal-focus-glow`} value={form.wave} onChange={(e) => update("wave", e.target.value)} placeholder="e.g. 2024-Q3-01" />
                </FormField>
              </Tooltip>
              <Tooltip content="Employee hire date for tenure tracking" position="top">
                <FormField label="Hire Date" required>
                  <input type="date" className={`${inputClass} teal-focus-glow`} value={form.hireDate} onChange={(e) => update("hireDate", e.target.value)} />
                </FormField>
              </Tooltip>
              <Tooltip content="Client or business unit" position="top">
                <FormField label="Account" required>
                  <select className={`${inputClass} teal-focus-glow`} value={form.accountId} onChange={(e) => update("accountId", e.target.value)}>
              ...
              <Tooltip content="Line of business within the account" position="top">
                <FormField label="LOB" required>
                  <select className={`${inputClass} teal-focus-glow`} value={form.lobId} onChange={(e) => update("lobId", e.target.value)} disabled={!form.accountId}>
                    <option value="">{form.accountId ? "Select LOB..." : "Select account first"}</option>
                    {filteredLOBs.map((l) => (<option key={l.id} value={l.id}>{l.title}</option>))}
                  </select>
                </FormField>
              </Tooltip>
              <Tooltip content="Office or location" position="top">
                <FormField label="Site" required>
                  <select className={`${inputClass} teal-focus-glow`} value={form.siteId} onChange={(e) => update("siteId", e.target.value)}>
        ...
        {/* Step 2: Incident Details - Two Column */}
        {step === 2 && (
          <div className="space-y-5 stagger-slide-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Tooltip content="Primary reason for attrition tracking" position="top">
                <FormField label="Attrition Category" required>
                  <select className={`${inputClass} teal-focus-glow`} value={form.attritionCategory} onChange={(e) => update("attritionCategory", e.target.value)}>
                    <option value="">Select category...</option>
                    {ATTRITION_CATEGORIES.map((c) => (<option key={c} value={c}>{c}</option>))}
                  </select>
                </FormField>
              </Tooltip>
              <Tooltip content="Specific reason within the category" position="top">
                <FormField label="Sub-Reason" required>
                  <select className={`${inputClass} teal-focus-glow`} value={form.subReason} onChange={(e) => update("subReason", e.target.value)} disabled={!form.attritionCategory}>
                    <option value="">{form.attritionCategory ? "Select sub-reason..." : "Select category first"}</option>
                    {(SUB_REASON_MAP[form.attritionCategory] || []).map((r) => (<option key={r} value={r}>{r}</option>))}
                  </select>
                </FormField>
              </Tooltip>
              <Tooltip content="Impact level of this incident" position="top">
                <FormField label="Severity Level" required>
                  <select className={`${inputClass} teal-focus-glow`} value={form.severityLevel} onChange={(e) => update("severityLevel", e.target.value as SeverityLevel)}>
                    {(["Low", "Medium", "High", "Critical"] as SeverityLevel[]).map((s) => (<option key={s} value={s}>{s}</option>))}
                  </select>
                </FormField>
              </Tooltip>
              <Tooltip content="Total hours missed from training" position="top">
                <FormField label="Hours Missed" required>
                  <input type="number" min="0" step="0.5" className={`${inputClass} font-mono teal-focus-glow`} value={form.totalMissedHours} onChange={(e) => update("totalMissedHours", parseFloat(e.target.value) || 0)} />
                </FormField>
              </Tooltip>
              <Tooltip content="When the incident occurred" position="top">
                <FormField label="Incident Date" required className="md:col-span-2">
                  <input type="date" className={`${inputClass} teal-focus-glow`} value={form.incidentDate} onChange={(e) => update("incidentDate", e.target.value)} />
                </FormField>
              </Tooltip>
            </div>
            <div className="flex gap-8 pt-2">
              <Tooltip content="Mark if supporting documents are needed" position="top">
                <Toggle label="Documentation Required" value={form.documentationRequired} onChange={(v) => update("documentationRequired", v)} />
              </Tooltip>
              <Tooltip content="Flag for management escalation" position="top">
                <Toggle label="Escalation Required" value={form.escalationRequired} onChange={(v) => update("escalationRequired", v)} />
              </Tooltip>
            </div>
        ...
        {/* Step 3: Manager & Notes - Two Column */}
        {step === 3 && (
          <div className="space-y-5 stagger-slide-in">
            ...
            <div className="glass-card bg-gradient-to-r from-teal-50/80 to-white border border-teal-200/50 rounded-xl px-4 py-3">
              <p className="text-xs font-semibold text-teal-700 uppercase tracking-wide mb-2 flex items-center gap-1">
                <User className="w-3 h-3" /> Submitting Trainer
              </p>
              ...
              <p className="text-teal-600 text-sm flex items-center gap-2 mt-0.5">
                ...
                <Briefcase className="w-3 h-3" /> {user?.jobTitle || "Trainer"}
              </p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Tooltip content="Direct supervisor or manager name" position="top">
                <FormField label="Training Manager Name" required>
                  <input type="text" className={`${inputClass} teal-focus-glow`} value={form.trainingManager} onChange={(e) => update("trainingManager", e.target.value)} placeholder="Manager Full Name" />
                </FormField>
              </Tooltip>
              <Tooltip content="Manager's email for notifications" position="top">
                <FormField label="Training Manager Email" required>
                  <input type="email" className={`${inputClass} teal-focus-glow`} value={form.trainingManagerEmail} onChange={(e) => update("trainingManagerEmail", e.target.value)} placeholder="manager@concentrix.com" />
                </FormField>
              </Tooltip>
            </div>
            <Tooltip content="Additional context or observations" position="top">
              <FormField label="Notes">
                <textarea className={`${inputClass} resize-none teal-focus-glow`} rows={4} value={form.notes} onChange={(e) => update("notes", e.target.value)} placeholder="Additional context, observations, or action items..." />
              </FormField>
            </Tooltip>
            <div className="glass-card bg-gray-50/80 border border-gray-200/50 rounded-xl p-4 space-y-3">
              <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide flex items-center gap-1">
                <FileText className="w-3 h-3" /> Case Summary
              </p>
        ...
        <div className="flex items-center justify-between pt-5 border-t border-gray-100">
          <button
            ...
            className="text-sm text-gray-600 hover:text-gray-900 flex items-center gap-1 font-medium transition-colors btn-press"
          >
          ...
          {step < 3 ? (
            <button
              onClick={nextStep}
              className="bg-gradient-teal hover:opacity-90 text-white text-sm font-semibold px-6 py-2.5 rounded-xl transition-all shadow-glow-teal flex items-center gap-2 btn-press"
            >
            ...
            <Tooltip content="Submit case and notify stakeholders" position="left">
              <button
                ...
                disabled={submitting}
                className="bg-gradient-teal hover:opacity-90 disabled:opacity-60 text-white text-sm font-semibold px-6 py-2.5 rounded-xl transition-all shadow-glow-teal flex items-center gap-2 btn-press"
              >
...
const inputClass = "w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm text-gray-800 focus:outline-none transition-all bg-white/80 backdrop-blur-sm disabled:bg-gray-50 disabled:text-gray-400";
...
function SectionTitle({ children, icon }: { children: React.ReactNode; icon: React.ReactNode }) {
  return (
    <h2 className="font-barlow-condensed font-semibold text-xl text-gray-900 tracking-wide flex items-center gap-2 mb-4">
      <span className="w-8 h-8 rounded-lg bg-teal-100 flex items-center justify-center text-teal-600">{icon}</span>
      {children}
    </h2>
  );
}
...
function Toggle({ label, value, onChange }: { label: string; value: boolean; onChange: (v: boolean) => void }) {
  return (
    <label className="flex items-center gap-3 cursor-pointer group">
      <div className={`w-11 h-6 rounded-full transition-all ${value ? "bg-gradient-teal shadow-glow-teal" : "bg-gray-300"} relative`} onClick={() => onChange(!value)}>
...
function Summary({ label, value, mono, highlight }: { label: string; value: string; mono?: boolean; highlight?: boolean }) {
  return (
    <div className="flex gap-2 items-baseline">
      ...
      <span className={`text-xs ${mono ? "font-mono" : ""} ${highlight ? "font-bold text-teal-700" : "text-gray-800"}`}>{value || "—"}</span>
    </div>
  );
}
