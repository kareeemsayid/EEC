...
import { fetchCases, AttritionCase } from "../../api/api";
...
import {
  Plus, MapPin, Search, Eye, AlertTriangle,
  Inbox, ChevronRight, RefreshCw, Clock, FolderOpen,
  Building2,
} from "lucide-react";
...
export default function TrainerDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<"All" | "Critical" | "Monitoring">("All");
  ...
  const loadData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const [casesResult, relData] = await Promise.all([
        fetchCases(),
  ...
  const filteredCases = useMemo(() => {
    ...
    if (activeTab === "Monitoring") list = list.filter(c => c.riskStatus === "Monitoring");
    if (search) {
    ...
    <div className="space-y-6 max-w-7xl mx-auto animate-fade-in">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <h1 className="text-2xl font-bold text-navy-900">My Training Cases</h1>
          <p className="text-sm text-gray-500 mt-1">{user?.displayName} · {activeCount} active / {totalCases} total</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        >
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-xs font-medium px-2 py-0.5 rounded-full bg-blue-50 text-eecblue">+8%</span>
          </div>
          <div className="text-3xl font-bold text-navy-900">
            {loading ? "—" : <CountUp value={totalCases} duration={600} />}
          </div>
          <p className="text-xs text-gray-500 font-medium uppercase tracking-wide mt-1">Total Cases</p>
        </motion.div>
      ...
      <div className="flex items-center gap-3 flex-wrap">
        {(["All", "Critical", "Monitoring"] as const).map(tab => (
          <button
            key={tab}
            onClick={() => setActiveTab(tab)}
            className="px-4 py-2 rounded-xl text-sm font-medium transition-all"
            style={activeTab === tab
              ? { background: "#1E3A5F", color: "white", boxShadow: "0 4px 12px rgba(30,58,95,0.2)" }
